...
class DetectItemHelper:
    ...
    def process(self):
        ...
        self.data_a_w_id = np.concatenate([np.array(range(self.ppl_num)).reshape(self.ppl_num, 1), self.a], axis=1)
    ...
    def recursive_cluster(self,cur_2groups):
        ...
        def cluster(cur_cluster_row):
            data_a = np.delete(self.data_a_w_id, 0, axis=1)[cur_cluster_row['group_mem_id'].astype(int), :]
            label = AgglomerativeClustering(n_clusters=2,linkage="complete").fit(data_a).labels_
            left = {'group_mem_id': self.data_a_w_id[cur_cluster_row['group_mem_id'], 0][np.where(label == 0)].astype(int)}
            right = {'group_mem_id': self.data_a_w_id[cur_cluster_row['group_mem_id'], 0][np.where(label == 1)].astype(int)}
            return {'left': left, 'right': right}

        if len(cur_2groups) == 0:
            cur_2groups = {'root': {'group_mem_id': self.data_a_w_id[:, 0].astype(int)}}
        for cluster_id, cur_cluster_row in cur_2groups.items():
            cur_cluster_output = cluster(cur_cluster_row)
            if (len(cur_cluster_output['left']['group_mem_id']) <= exp_default_para['group_mem_size_cri']) or \
                    (len(cur_cluster_output['right']['group_mem_id']) <= exp_default_para['group_mem_size_cri']):
    ...
    def group_check(self):
        for cur_group in self.clustering_res:
            cur_group['group_ab_item'] = np.array(np.where(np.sum(a=self.a[cur_group['group_mem_id']], axis=0) > (len(cur_group['group_mem_id']) / 2))).flatten()
            if len(cur_group['group_ab_item']) > exp_default_para['ab_item_size_cri']:
    ...
    def vote(self):
        ...
        for cur_group in self.cleaned_cluster_res:
            ...
            cur_ci_newp = np.nanmean(np.delete(self.crt[group_mem_id, :], group_ab_item, axis=1))
            cur_nci_ewp = np.nanmean(np.delete(self.crt[:, group_ab_item], group_mem_id, axis=0))
